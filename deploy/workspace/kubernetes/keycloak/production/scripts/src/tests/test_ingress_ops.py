    ...
    "items": [
        {
            ...
            "spec": {
                "routes": [{"match": "Host(`old.example.com`)"}]
            },
        }
    ]
}).encode()
    ...
    "items": [
        {
            "metadata": {"name": "dtaas-tls"},
            "spec": {
                "routes": [{"match": "Host(`new.example.com`)"}]
            },
        }
    ]
}).encode()
...
class TestPatchIngressroutes:
    ...
    def test_dry_run_prints_patched_yaml(self, capsys: pytest.CaptureFixture) -> None:
        ...
        with patch("src.ingress_ops.kubectl", return_value=make_proc(stdout=_SINGLE_ITEM_JSON)):
    ...
    def test_exits_on_kubectl_error(self) -> None:
        ...
        with patch("src.ingress_ops.kubectl", return_value=make_proc(returncode=1, stderr=b"err")):
