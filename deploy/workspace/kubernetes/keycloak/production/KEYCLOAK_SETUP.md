...
```bash
kubectl rollout restart deployment -n dtaas-workspace
```
...
- Ensure the Keycloak service is running: `kubectl get pods -n dtaas-workspace`
- Check Keycloak logs: `kubectl logs -n dtaas-workspace deploy/keycloak`
...
- Check traefik-forward-auth logs:
  `kubectl logs -n dtaas-workspace deploy/traefik-forward-auth`
- Verify environment variables are set correctly
- Ensure Keycloak is reachable from the traefik-forward-auth pod
